...
export const ARCHETYPES: Record<string, { name: string; emoji: string; color: string; description: string; traits: string[] }> = {
  ...
  tabber: { name: 'Taskpiler', emoji: '📦', color: '#FDA4AF', description: 'You do not finish tasks. You collect them. Every conversation adds another item to a pile that has achieved geological significance. You open tabs the way other people open doors — constantly, automatically, and with no intention of closing them. Two hundred and forty-seven tabs. Each one "important." Each one open since a date that predates your current hairstyle. You do not have a to-do list. You have a to-do landfill. Somewhere under layer thirty-seven is the original task that started all of this. It will never be found. It will never be finished. But it will absolutely never be deleted, because what if you need it someday?', traits: ['Task hoarder', 'Cannot let go', 'Pile over progress'] },
  ...
  sherlock: { name: 'Sherlock', emoji: '🔍', color: '#67E8F9', description: 'You once Googled something your agent said, found the exact same answer, and still asked a third time just to be safe. Every output is a suspect. Every source needs a source. You have asked your agent to verify its own verification. Trust is not a feature — it is a bug, and you have patched every version. If your birthday showed up on a calendar you did not personally audit, you would question it.', traits: ['Chronically suspicious', 'Verification-obsessed', 'Trust issues'] },
